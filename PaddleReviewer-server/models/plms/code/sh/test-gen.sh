...
python PaddleReviewer-server/models/plms/code/run_test_gen.py  \
  --model_type t5 \
  --add_lang_ids \
  --train_epochs 30 \
  --config_name ${mnt_dir}/codereviewer \
  --tokenizer_path ${mnt_dir}/codereviewer \
  --model_name_or_path ${mnt_dir}/codereviewer \
  --load_model_path ${mnt_dir}/codereviewer \
  --output_dir ${mnt_dir}/outputs \
  --eval_file ${mnt_dir}/Comment_Generation/msg-test.jsonl \
  --max_source_length 300 \
  --max_target_length 128 \
  --eval_batch_size 12 \
  --mask_rate 0.15 \
